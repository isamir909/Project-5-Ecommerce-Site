...
const updateData = async function(req,res){
    ...
    //    if(data.address.billing.city){ if(!validator.isAlpha(data.address.billing.city))return res.status(400).send({status:false,msg:'city name in billing must be between a-z or A-Z'})}    
      if(data.address){if(data.address.billing){if(data.address.billing.city){if(!validator.isAlpha(data.address.billing.city))return res.status(400).send({status:false,msg:'city name in billing must be between a-z or A-Z'})}}
